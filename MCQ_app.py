...
def predict():
    ...
    text="generate only"+ QA_count+QA_difficulty+"Multi choice questions on "+course_name+"for"+btech_branch+"branch starting with *Question*."
    ...
    answers=palm.generate_text(prompt='generate only options for:'+questions_string)
    answers_list=[i for i in answers.result.split('\n')]
    questions = [question.strip() for question in questions_string.split("Question") if question]
    questions_list=[]
    # answers_list=[]
    y=''
